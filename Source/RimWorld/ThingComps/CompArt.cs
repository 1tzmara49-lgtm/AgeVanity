...
public class CompArt : ThingComp
{
    ...
    {
        // This happens if quality gets set multiple times (dev generate unique weapons)
        if (!titleInt.NullOrEmpty())
            return;
	...
	protected virtual string GenerateTitle(ArtGenerationContext context)
	{
        ...
        return GenText.CapitalizeAsTitle((string)taleRef.GenerateText(TextGenerationPurpose.ArtName, Props.nameMaker));
	}
}}
