...
public class Fire : AttachableThing, ISizeReporter
{
    ...
    public override string InspectStringAddon => "Burning".Translate() + " (" + "FireSizeLower".Translate( (fireSize*100).ToString("F0") ) + ")";
    ...
    public override int UpdateRateTicks => GenTicks.MaxTickInterval;

    // IThingGlower
    public override bool ShouldBeLitNow() => true;
    ...
    public override void AttachTo(Thing newParent)
    {
        ...
        if (newParent is Pawn p)
            TaleRecorder.RecordTale(TaleDefOf.WasOnFire, p);
    }

    protected override void Tick()
    {
        // Maintain sustainer - Since "re-fogging" isn't currently a thing, we don't bother dealing with fires that are now invisible.
    ...
    protected override void TickInterval(int delta)
    {
        ...
        if ((sustainer == null || sustainer.Ended) && !Position.Fogged(Map))
        {
        ...
        // Do complex calcs
        if (this.IsHashIntervalTick(ComplexCalcsInterval, delta))
            DoComplexCalcs();
    ...
    private void SpawnSmokeParticles()
    {
        if( fireCount < FireCountParticlesOff )
            FleckMaker.ThrowSmoke(DrawPos, Map, fireSize);

        if( fireSize > 0.5f && parent == null )
            FleckMaker.ThrowFireGlow(Position.ToVector3Shifted(), Map, fireSize);
    ...
    private void DoComplexCalcs()
    {
        ...
        else if( parent == null )
        {
            if( Position.TerrainFlammableNow(Map) )
                flammabilityMax = Position.TerrainFlammability(Map);
            ...
            for( int i = 0; i < cellThings.Count; i++ )
            {
                ...
                if( parent == null && fireSize > MinSizeForIgniteMovables && cellThings[i].def.category == ThingCategory.Pawn  && Rand.Chance(FireUtility.ChanceToAttachFireCumulative(cellThings[i], ComplexCalcsInterval)))
                    cellThings[i].TryAttachFire(fireSize * 0.2f, instigator);
            }
        }
        else
        {
        ...
        if( Spawned )
        {
            ...
            if( Rand.Value < 0.4f )
            {
                ...
                WeatherBuildupUtility.AddSnowRadial(Position, Map, snowClearRadius, -(fireSize * SnowClearDepthFactor));
            }
            ...
            var vacuum = FireUtility.GetEffectiveVacuumForFire(Position, Map);
            ...
            if (vacuum > 0)
                TakeDamage(new DamageInfo(DamageDefOf.Extinguish, VacuumExtinguishDamage * vacuum));
    ...
    private void DoFireDamage( Thing targ )
    {
        ...
        if( targ is Pawn p )
        {
            ...
            DamageInfo dinfo = new DamageInfo(DamageDefOf.Flame, damAmount, instigator: instigator ?? this);
            dinfo.SetBodyRegion( depth: BodyPartDepth.Outside );
            targ.TakeDamage(dinfo).AssociateWithLog(log);

            //Damage a random apparel
            if( p.apparel != null && p.apparel.WornApparel.TryRandomElement(out var ap) )
                ap.TakeDamage(new DamageInfo(DamageDefOf.Flame, damAmount, instigator: instigator ?? this));
        }
        else
        {
            targ.TakeDamage(new DamageInfo(DamageDefOf.Flame, damAmount, instigator: instigator ?? this));
        }
    }
    ...
    protected void TrySpread()
    {
        ...
        if( Rand.Chance(FireUtility.ChanceToStartFireIn(targLoc, Map)) )
        {
            ...
            else
            {
                ...
                FireUtility.TryStartFireIn(targLoc, Map, MinFireSize, instigator);
            }
        }
    }
}
